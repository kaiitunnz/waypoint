...
and acceptance criteria. With `{{spec_ref}}` empty (a re-spec of a ticket first found
infeasible), author a fresh PRD guided by the note. With no `kind=respec` note, author
fresh from the request above.
