...
A message sent from another session is held, reported as `"send": "held"`,
while the target is in Focus (its human releases or cancels it) or waiting on a
dialog (it is delivered once the human answers).
