...
`backend/` contains the FastAPI service. Core code lives in `backend/src/waypoint/`, with API, runtime, auth, storage, and CLI modules split by concern. The `backend/src/waypoint/telemetry/` package holds the backend-neutral usage-telemetry subsystem (fact contract, ingestion, rollups, aggregation) behind the `/api/telemetry` surface and the `/telemetry` dashboard page; see [`docs/telemetry.md`](docs/telemetry.md). The `backend/src/waypoint/notifications/` package holds the backend-neutral, opt-in notification center (durable outbox, pluggable outbound channels, Telegram in v1) that pushes new inbox items and actionable session interactions off-host; backends attach a normalized `interaction` envelope to event metadata and the runtime/notification path never branches on backend id — see [`docs/telegram-notifications.md`](docs/telegram-notifications.md). The `backend/src/waypoint/usage_providers/` package holds the backend-neutral, opt-in usage-provider subsystem (provider protocol, per-type config dispatch, runtime-owned refresh service, durable snapshot store, Lumid in v1) that surfaces session-independent account usage on `/api/usage` and the home usage panel; the runtime, dashboard, API, and telemetry paths never branch on a provider id — see [`docs/usage-providers.md`](docs/usage-providers.md). Backend tests live in `backend/tests/`. `frontend/` contains the Next.js 15 PWA; routes live in `frontend/src/app/` (including the `telemetry/` dashboard), shared UI in `frontend/src/components/`, and client helpers in `frontend/src/lib/`. The Codex backend uses the `openai-codex` Python SDK from PyPI (pinned in `backend/pyproject.toml`); the Codex collaboration-mode templates it needs are vendored under `backend/src/waypoint/backends/codex/collaboration_mode_templates/` since the wheel does not ship them.
...
The repo's `.agents/skills/` are auto-installed only into the personal assistant's workspace. To let *any* coding session drive Waypoint sessions through the `waypoint` CLI (`waypoint`), spawn child Waypoint sessions (`waypoint-subagents`), message other live sessions (`waypoint-comms`), orchestrate large parallel batch jobs over the blackboard (`waypoint-workqueue`), run a multi-role software-engineering organization that drives a product through its whole lifecycle (`waypoint-crew`), run one session as an autonomous product owner that drains a project's ticket board for its lifetime (`waypoint-manager`), commit safely in the shared multi-session working tree (`waypoint-worktree`), and run open-ended optimization research over isolated Waypoint search sessions (`waypoint-research`), install those skills into the cross-agent global skills directory (`~/.agents/skills` by default) with `scripts/install_skills.sh install` (or `waypointctl skills install`); all eight ship in the default selection. The installer symlinks by default so the skills track the repo; pass `--skill-dir` / `WAYPOINT_SKILLS_DIR` to target an agent-specific directory (e.g. `~/.claude/skills`, `~/.codex/skills`), `--skill`/`--all` to widen the selection, and `--copy` for a detached snapshot. `status` and `uninstall` round-trip it; `uninstall` only removes symlinks the installer created, never a real or copied directory.
