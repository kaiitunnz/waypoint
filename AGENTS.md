...
- Floating chrome only (sticky composer, docks, popovers, modals, tooltips, scroll floaters) uses the glass recipe: `background: var(--glass-bg)` (or `--glass-bg-thick` for sheets that carry scrolling text), `backdrop-filter: var(--glass-blur)` (+ `-webkit-` prefix), `border: 1px solid var(--glass-line)`, `box-shadow: inset 0 1px 0 var(--glass-hi), var(--shadow)`, `border-radius: var(--radius-glass)`. Never put glass on in-flow content, and never float the app bar.
