...
def _runtime_resolving(target: SshLaunchTargetConfig | None) -> "SessionRuntime":
    ...
    The SSH delete path only touches the launch-target resolver, so a stub
    keeps the test off a real SessionRuntime.
    """
...
@pytest.mark.asyncio
async def test_delete_thread_routes_through_ssh(
    plugin: ClaudeCodePlugin, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    assert len(calls) == 1
    cmd = calls[0]
    assert f"{thread_id}.jsonl" in cmd
    assert "${CLAUDE_CONFIG_DIR:-$HOME/.claude}/projects/" in cmd
    assert "rm -f" in cmd
...
async def test_delete_thread_remote_reports_false_when_nothing_removed(
    ...
    deleted = await plugin.delete_thread(
        _runtime_resolving(target), "00000000-0000-0000-0000-000000000002", "t"
    )
...
@pytest.mark.asyncio
async def test_delete_thread_remote_returns_false_when_target_missing(
    plugin: ClaudeCodePlugin,
) -> None:
    deleted = await plugin.delete_thread(
        _runtime_resolving(None), "00000000-0000-0000-0000-000000000003", "gone"
    )
    assert deleted is False
