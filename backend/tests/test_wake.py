...
async def test_self_inbox_mutation_does_not_wake_but_human_does(
    ...
    # The manager filing/answering its own item: no self-wake.
    await runtime._dispatch_subscription_wakes(
        channel=None, is_inbox=True, actor_session_id="codex-mgr"
    )
    assert calls == []

    # A human answer carries no session id: the manager is woken.
    await runtime._dispatch_subscription_wakes(
        channel=None, is_inbox=True, actor_session_id=None
    )
...
async def test_running_subscriber_burst_coalesces_and_fires_on_edge(
    ...
    await runtime._drain_pending_wakes({"codex-sub"})
    assert calls == [("codex-sub", WAKE_INPUT_TEXT)]
...
async def test_post_board_entry_wires_dispatch(tmp_path, monkeypatch) -> None:
    ...
    # The dispatch is a tracked background task; await it deterministically.
    if runtime._wake_tasks:
