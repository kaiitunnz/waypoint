...
async def test_git_status_classifies_changes(
    ...
    status = await git_status(workspace_fs, str(tmp_path))
...
async def test_git_status_translates_subdir_prefix(
    tmp_path: Path, workspace_fs: WorkspaceFilesystem
) -> None:
    _init_repo(tmp_path)
    ...
    status = await git_status(workspace_fs, str(sub))
    assert status is not None
...
async def test_git_file_diff_tracked_modification(
    tmp_path: Path, workspace_fs: WorkspaceFilesystem
) -> None:
    _init_repo(tmp_path)
    ...
    preview = await git_file_diff(workspace_fs, str(tmp_path), "f.txt", staged=False)
...
async def test_git_file_diff_includes_full_file_context(
    tmp_path: Path, workspace_fs: WorkspaceFilesystem
) -> None:
    _init_repo(tmp_path)
...
async def test_git_file_diff_staged_only(
    tmp_path: Path, workspace_fs: WorkspaceFilesystem
) -> None:
    _init_repo(tmp_path)
    (tmp_path / "f.txt").write_text("a\n", encoding="utf-8")
    ...
    staged = await git_file_diff(workspace_fs, str(tmp_path), "f.txt", staged=True)
    assert staged is not None
    assert "+staged" in staged.files[0].diff
    assert "unstaged" not in staged.files[0].diff


async def test_git_file_diff_untracked_synthesizes_add(
    tmp_path: Path, workspace_fs: WorkspaceFilesystem
) -> None:
    _init_repo(tmp_path)
    ...
    preview = await git_file_diff(workspace_fs, str(tmp_path), "new.txt", staged=False)
    assert preview is not None
    assert preview.files[0].change_type == "add"
    assert "+hello" in preview.files[0].diff


async def test_git_file_diff_unchanged_returns_none(
    tmp_path: Path, workspace_fs: WorkspaceFilesystem
) -> None:
    _init_repo(tmp_path)
    (tmp_path / "f.txt").write_text("a\n", encoding="utf-8")
    ...
    assert (
        await git_file_diff(workspace_fs, str(tmp_path), "f.txt", staged=False) is None
    )


async def test_git_status_reports_branch(
    tmp_path: Path, workspace_fs: WorkspaceFilesystem
) -> None:
    _init_repo(tmp_path)
...
async def test_git_status_detached_on_tag(
    tmp_path: Path, workspace_fs: WorkspaceFilesystem
) -> None:
    _init_repo(tmp_path)
    (tmp_path / "f.txt").write_text("a\n", encoding="utf-8")
...
async def test_git_status_detached_on_commit(
    tmp_path: Path, workspace_fs: WorkspaceFilesystem
) -> None:
    _init_repo(tmp_path)
    (tmp_path / "f.txt").write_text("a\n", encoding="utf-8")
...
async def test_git_list_files_includes_untracked_skips_ignored(
    tmp_path: Path, workspace_fs: WorkspaceFilesystem
) -> None:
    _init_repo(tmp_path)
    ...
    listed = await git_list_files(workspace_fs, str(tmp_path))
...
async def test_git_list_files_non_repo_returns_none(
    tmp_path: Path, workspace_fs: WorkspaceFilesystem
) -> None:
    assert await git_list_files(workspace_fs, str(tmp_path)) is None
...
async def test_git_status_unborn_branch(
    tmp_path: Path, workspace_fs: WorkspaceFilesystem
) -> None:
    _init_repo(tmp_path)
    _git(tmp_path, "symbolic-ref", "HEAD", "refs/heads/trunk")
    (tmp_path / "new.txt").write_text("x\n", encoding="utf-8")
    status = await git_status(workspace_fs, str(tmp_path))
    assert status is not None
    assert status.branch == "trunk"
    assert status.detached is False
    assert [entry.path for entry in status.files] == ["new.txt"]


async def test_git_status_expands_tilde_base(
    tmp_path: Path, workspace_fs: WorkspaceFilesystem, monkeypatch
) -> None:
    repo = tmp_path / "repo"
    repo.mkdir()
    _init_repo(repo)
    monkeypatch.setenv("HOME", str(tmp_path))
    status = await git_status(workspace_fs, "~/repo")
    assert status is not None
