...
| `supports_set_permission_mode_with_restart` | `bool` | The transport applies a permission-mode change by restarting and resuming the session (claude_tty); the composer asks for confirmation first. |
...
| `terminal_interactive` | `bool` | The pane accepts free-form keyboard input (xterm `onData`), the compose drawer, and mouse forwarding. `True` for `tmux`; `False` for `claude_tty`, whose input surface is the structured chat. |
| `terminal_key_injection` | `bool` | The pane accepts discrete injected input — the key-bar chips (Esc/Enter/arrows/^C) and scroll-wheel events — and the frontend can unlock it per view for typing and mouse input. Implied by `terminal_interactive`; also `True` for `claude_tty` as an escape hatch for driving the TUI through unexpected dialogs and scrolling its history. Gates the frontend key-bar and the WS `input` frame; `input_submit` needs `terminal_interactive`. `interactive: true` on the client's `hello` frame forwards the TUI's mouse-tracking modes for that connection. |
| `terminal_resizable` | `bool` | The pane's dimensions can be driven by the client viewport. `True` for `tmux`; `False` for `claude_tty` (fixed server-side grid managed by the relaunch cycle). |
...
`opus`/`claude-opus-5-5` (200K), and the transcript records only the resolved base
...
A fourth key, `capture_origin`, tags every attachment those sinks create on
the event with an `AttachmentOrigin` (`task_output` for task-notification
reports), which the session Files panel shows as a label. It is popped the
same way.
