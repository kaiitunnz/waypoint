...
| `terminal_key_injection` | `bool` | The pane accepts discrete injected input — the key-bar chips (Esc/Enter/arrows/^C) and scroll-wheel events — without opening it to free-form typing. Implied by `terminal_interactive`; also `True` for `claude_tty` as an escape hatch for driving the TUI through unexpected dialogs and scrolling its history. Gates the frontend key-bar and the WS `input` frame (whereas `input_submit` needs full `terminal_interactive`). `interactive: true` on the client's `hello` frame makes the server forward the TUI's mouse-tracking modes for that connection, so clicks reach the pane. |
