...
or approval-pending `waiting_input` is held in the session's held-messages queue and
fires once the session is idle, so it never interrupts an active turn or injects
while an approval is pending. A session holds at most one wake, the held wake
survives a backend restart, and a held wake whose session exits fires after the
session is resumed. A wake into `exited` or `error` is dropped — a stopped session
is not resurrected by a board post; an explicit resume, or a pending liveness
self-wake (which delivers through the input path), recovers it.
...
- Events that arrive while the backend is down do not wake the manager (the wake
  driver is the backend). They sit durably on the board. While a ticket is in flight, the pending liveness self-wake (durable,
