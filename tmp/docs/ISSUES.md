...
- [-] **List attachable headless sessions.** _Skipped for now — too brittle._ Both CLIs persist transcripts to disk (`~/.claude/projects/<encoded-cwd>/<uuid>.jsonl`, `~/.codex/sessions/<yyyy>/<mm>/<dd>/rollout-<ts>-<uuid>.jsonl`), but adoption requires detecting whether a CLI is currently appending to that transcript (otherwise concurrent writers corrupt the JSONL) and then either killing the live process or refusing the adoption — neither of which has a clean cross-host story today.
